...
def main():
    ...
    # 加载自己的点云文件
    file_name = '/home/yu/3D Point Cloud Processing/DataSet/MyDataSet/airplane/train/airplane_0137.ply'
    point_cloud_pynt = PyntCloud.from_file(file_name)
    ...
    start = time.time()
    filtered_cloud = voxel_filter(point_cloud_pynt.points, 100, random_pick=True)
    ...
    start = time.time()
    filtered_cloud = voxel_filter(point_cloud_pynt.points, 100, random_pick=False)
    point_cloud_o3d.points = o3d.utility.Vector3dVector(filtered_cloud)
    print("Down sample takes {}s.".format(time.time() - start))
    # 显示滤波后的点云
    o3d.visualization.draw_geometries([point_cloud_o3d])
