...
public:
    ...
    Node(int& ax, double v, Node* l, Node* r, std::vector<int>& point_idx)
    {
        axis = ax;
        value = v;
        left = l;
...
void sortKeysbyValues(const std::vector<int>& point_indices,
                      ...
                      std::vector<int>& key_sorted, std::vector<double>& value_sorted)
{
                  ...
                  int axis, int leaf_size)
{
    ...
    if (point_indices.size() > leaf_size)
    {
        ...
        std::vector<double> value_sorted;
                            ...
                            int axis, int leaf_size)
{
    ...
    if (point_indices.size() > leaf_size)
    {
        ...
        double median = quickSelect(dbCrop, dbCrop.size());
        root->value = median;
...
Node* KDTreeConstruction(std::vector<std::vector<double>>& db, int leaf_size)
{
    int N = db.size();
