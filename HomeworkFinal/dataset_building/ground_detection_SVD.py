...
def pcd_preprocessing(data, match_image_range=False):
    if match_image_range:
        x_range_filter = data[:, 0] > 5
        data = data[x_range_filter]
...
def ground_detection_on3segs(pcd_points, main_dist=20, max_iter=6, threshold_dist=0.18, match_image_range=False):
    if match_image_range:
        x_max = 150
        segments_x = [5, main_dist, x_max]
    else:
...
if __name__ == "__main__":
    start = time.time()
    path = "/media/yu/学习/DataSet/KITTI/object/training/velodyne/"
    points = raw_kitti_bin_reader(path, "000130")[:, :3]
    ...
    start = time.time()
    clusters = foreground_pcd.cluster_dbscan(0.2, 20, print_progress=True)
